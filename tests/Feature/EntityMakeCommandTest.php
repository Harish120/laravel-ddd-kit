...
it('generates a companion Pest test alongside a plain entity', function (): void {
    config()->set('ddd.generate_tests', true);
    config()->set('ddd.tests_path', $this->tempRoot.'/tests');

    $this->artisan('ddd:entity', ['name' => 'Contact/Lead'])->assertSuccessful();

    $file = "{$this->tempRoot}/tests/Unit/Domains/Contact/Domain/Entities/LeadTest.php";

    expect(File::exists($file))->toBeTrue()
        ->and(File::get($file))->toContain("use {$this->namespace}\\Contact\\Domain\\Entities\\Lead;")
        ->and(File::get($file))->toContain("Lead::create('1')");

    exec('php -l '.escapeshellarg($file), result_code: $exitCode, output: $output);
    expect($exitCode)->toBe(0, implode("\n", $output));
});

it('generates a companion Pest test alongside an aggregate root', function (): void {
    config()->set('ddd.generate_tests', true);
    config()->set('ddd.tests_path', $this->tempRoot.'/tests');
    ...
    expect(File::exists($file))->toBeTrue()
        ->and(File::get($file))->toContain('pullDomainEvents()');
...
it('does not generate a companion test when disabled', function (): void {
    ...
    expect(File::exists("{$this->tempRoot}/tests/Unit/Domains/Contact/Domain/Entities/LeadTest.php"))->toBeFalse();
});
